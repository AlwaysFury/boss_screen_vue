<template>
	<el-dialog v-model="showDialog" title="新增标签">
		<el-form :model="tagDialogData" label-width="100">
			...
			<el-form-item label="标签">
				...
				<el-input v-else v-model="tagDialogData.customTag"></el-input>
			</el-form-item>
		</el-form>
		<template #footer>
			<div class="dialog-action">
				<el-button type="primary" @click="onConfirmTag">确认</el-button>
...
async function remoteMethod(tag_name) {
	if (!tag_name) {
		...
		tagData.value = await fn({ tag_name });
	}
}

function onConfirmTag() {
	const target =
		tagDialogData.value.type == 1
			? tagData.value.find((item) => item.key == tagDialogData.value.tag)
			: {};
